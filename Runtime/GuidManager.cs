...
namespace Celezt.SaveSystem
{
	...
	public class GuidManager
    {
		...
		private struct GuidInfo
        {
            ...
            public GuidInfo(SaveBehaviour comp)
            {
        ...
        public static bool Add(SaveBehaviour saveBehaviour) => Instance.InternalAdd(saveBehaviour);
        ...
        {
            Guid guid = saveBehaviour.GetGuid();
            GuidInfo info = new GuidInfo(saveBehaviour);
            ...
            if (existingInfo.GameObject != null && existingInfo.GameObject != saveBehaviour.gameObject)
            {
                // normally, a duplicate GUID is a big problem, means you won't necessarily be referencing what you expect
                if (Application.isPlaying)
                {
                    Debug.AssertFormat(false, saveBehaviour,
                        ...
                        (_guidToObjectMap[guid].GameObject != null ? _guidToObjectMap[guid].GameObject.name : "NULL"), (saveBehaviour != null ? saveBehaviour.name : "NULL"));
                }
                else
                {
                    ...
                    Debug.LogWarningFormat(saveBehaviour, "Guid Collision Detected while creating {0}.\nAssigning new Guid.",
                        (saveBehaviour != null ? saveBehaviour.name : "NULL"));
                }
